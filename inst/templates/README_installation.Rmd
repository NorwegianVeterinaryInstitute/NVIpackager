...
To install (or update) the `r NVIpkg_inline` package with vignettes, you will need to first install some additional R-packages needed for creating the vignettes. Check README to see which vignettes are available and eventually install the packages: `rmarkdown`, `knitr`, `R.rsp`, and `NVIrpackages` (from GitHub) if they are missing. To install the package with the vignettes, run the following code:
